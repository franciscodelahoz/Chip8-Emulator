...
import type { DisplayPosition, EmulatorFontAppearance } from '../types/emulator';
        ...
        for (let plane = 0; plane < 2; plane += 1) {
          ...
          for (let rows = 0; rows < spriteHeight; rows += 1) {
            for (let columns = 0; columns < spriteWidth; columns += 1) {
              let pixelValue =
                n === 0 ?
                  (this.memory[i + rows * 2 + (columns > 7 ? 1 : 0)] >> (7 - (columns % 8))) & 1 :
              ...
              const setPixel = this.displayInstance.setPixel(position, pixelValue, plane);
